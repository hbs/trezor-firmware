use crate::ui::{
    ...
    layout_eckhart::component::{
        Button, VerticalMenuScreen, VerticalMenuScreenMsg, MENU_MAX_ITEMS,
    },
    shape::Renderer,
};
...
impl DeviceMenuScreen {
    ...
    fn handle_back_navigation(&mut self, ctx: &mut EventCtx) -> Option<DeviceMenuMsg> {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: Event) -> Option<Self::Msg> {
        ...
        match self.menu_stack[self.active_menu].0.event(ctx, event) {
            ...
            Some(VerticalMenuScreenMsg::Back) => {
                return self.handle_back_navigation(ctx);
            }
